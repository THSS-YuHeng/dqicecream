...
public class DataRepair {
	...
	public HashMap<String, LinkedList<Tuple>> correctTable;			//单行数据处理结果      ***使用“ ”作为错误的数据填充***
	...
	public void excute() {
		...
		for (Tuple tuple : tuples) {
			...
			if (!edata.errorFlagSet.get(rawAttrs.STADD_INDEX)&&
				...
				if (STADDSTNUMAPMTValidator.test(
...
//					tuple.setValue(rawAttrs.AGE, "  ");
		...
		for (String	key : errorTable.keySet()) {
			...
			for (int i = edata.errorFlagSet.nextSetBit(0); i >= 0; i = edata.errorFlagSet.nextSetBit(i+1)) {
				...
				Tuple specific = correctTable.get(edata.dataTuple.getValue(rawAttrs.CUID)).getFirst();
